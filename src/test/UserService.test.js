...
describe("getDetailUser", () => {
  it("should fetch details user successfully", async () => {
    const result = await UserService.getDetailsUser("664b678e48aa92aa6e00d3d3");
    expect(result).toHaveProperty("status", "OK");
    expect(result).toHaveProperty("message", "SUCESS");
  });

  it("should fetch user failed in case user is not exist", async () => {
    const result = await UserService.getDetailsUser("60f3b3b3b3b3b3b3b3b3b3b3");
  ...
  it("should fetch user failed in case user id is invalid format", async () => {
    ...
    jest.spyOn(UserService, "getDetailsUser").mockImplementation(async (id) => {
      if (id === invalidId) {
        throw new Error(
...
describe("refreshToken", () => {
  ...
  it("should refresh token is failed", async () => {
    ...
    const result = await UserService.loginUser(data);
    if (result.status === "OK") {
      const data = await JwtService.refreshTokenJwtService(result.access_token);
